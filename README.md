...
- **The atlas:** [`docs/atlas.md`](docs/atlas.md) — a calibration sweep of 80
  ...
  occupied realm, the town that freed itself — each one click away with its own
...
> `conduit_boom`, bloc `crown`/`temple`/`magnate`/`dominion`) stay stable — the
> data is the anchor, the register is the skin — and render in the new terms.
...
| `wealth` | 0–100 | **emergent** blend of retained extraction, artifice value-add, and trade income + a dialable legacy gradient term, scaled by the region's artifice index `A` — so total realm wealth is no longer conserved (a boom grows the pie, a trade war shrinks it) |
...
| `wealth` / `on_grid` / `arcane_service_index` | — | mirror their region |
