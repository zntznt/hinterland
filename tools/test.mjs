...
// Batch multiple genEngine calls through the worker pool.
// Usage: const results = await batch([hash1, hash2, ...]);
async function batch(hashes) {
  if (!hashes.length) return [];
  return pool.dispatch(hashes);
}
...
// comments elsewhere in this file claiming it ran. Its 284 assertions had therefore
// been inert for as long as that was true, and had rotted accordingly: its fertility
// mirror still carried the pre-5beea32 coefficients (0.56/0.30/0.10 with no biome
// term) and its biome rule still opened forest at rainfall 68 rather than 48.
//
// 263 of its 270 distinct checks were already enforced by the LIVE validate() in
// stress.mjs. The four that were not have been moved there, two of them corrected
// on the way, because as written they failed against the current engine.
// Nothing was lost by deleting it; something was gained by not pretending it ran.
...
const Aw = await genEngine("#seed=alpha&regions=24&relax=3&bias=70&we=80&wf=5&wt=5&wg=10");
...
const Acap = await genEngine(BASE + "&cx=150&cy=150");
...
const Bseed = await genEngine(BASE.replace("alpha", "beta"));
...

{
  ...
  const N = 25;
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=g1-${i}&regions=24`)).gj;
...
const RA0 = await genEngine("#seed=alpha&regions=24&ep=0");
const RA10 = await genEngine("#seed=alpha&regions=24&ep=10");
...

{
  ...
  const N = 20;
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=g4-${i}&regions=24`)).gj;
...

{
  ...
  const allAccess = [];
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=wa-${i}&regions=24&ep=10`)).gj;
...

{
  ...
  {
    const Z = await genEngine("#seed=h1-init&regions=30&ep=0");
  ...
  for (let i = 0; i < N; i++) {
    const R = await genEngine(`#seed=h1-${i}&regions=26&ep=10`);
    const F = R.gj.hinterland.findings;
...

{
  ...
  for (let i = 0; i < N; i++) {
    const op = await genEngine(`#seed=p2h-${i}&regions=26&ep=10`);
    const cl = await genEngine(`#seed=p2h-${i}&regions=26&ep=10&hb=0`);
  ...
  const B2 = await genEngine("#seed=alpha&regions=24&ep=10&iq=45&hb=1");
...

{
  ...
  const g0 = (await genEngine("#seed=m1-10&regions=34&ep=0")).gj; // the world that used to drown its towns
...

{
  ...
  const shAdv = [];
  const _h2066 = Array.from({length: N}, (_, i) => `#seed=s1-${i}&regions=${18 + (i % 30)}&ep=${i % 11}&gt=${(i * 13) % 101}&db=${(i * 17) % 101}`);
  const _r2066 = await batch(_h2066);
  for (let i = 0; i < N; i++) {
    const R = _r2066[i];
    const F = R.gj.hinterland.findings;
    const P = regionsOf(R.gj).map(f => f.properties);
  ...
  const K0 = await genEngine("#seed=sky-static&regions=24&ep=0"), K10 = await genEngine("#seed=sky-static&regions=24&ep=10");
...

{
  ...
  const N = 12;
  for (let i = 0; i < N; i++) {
    const R = await genEngine(`#seed=e5-${i}&regions=24&ep=10`);
  ...
  const A0 = (await genEngine("#seed=e5-0&regions=24&ep=0")).gj, A10 = (await genEngine("#seed=e5-0&regions=24&ep=10")).gj;
...
const _f2N = 20;
const _f2Hashes = Array.from({ length: _f2N }, (_, i) => `#seed=f2-${i}&regions=24&ep=10`);
const _f2Results = await batch(_f2Hashes);
...

{
  ...
  for (let i = 0; i < _f2N; i++) {
    const R = _f2Results[i];
    const evs = R.gj.hinterland.events || [];
...

{
  ...
  const N = 40;
  const _h = Array.from({length: N}, (_, i) => `#seed=f1-${i}&regions=24&ep=10`);
  const _r = await batch(_h);
  for (let i = 0; i < N; i++) {
    const R = _r[i];
    const evs = R.gj.hinterland.events || [];
...

{
  ...
  const N = 20;
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=p1-${i}&regions=24`)).gj;
    const R = regionsOf(g).map(f => f.properties);
...

{
  ...
  const N = 20;
  const _h = Array.from({length: N}, (_, i) => `#seed=g3-${i}&regions=24`);
  const _r = await batch(_h);
  for (let i = 0; i < N; i++) {
    const g = _r[i].gj;
    const R = regionsOf(g).map(f => f.properties);
...
const R1S = { rivN: 0, seaMouth: 0, borderMouth: 0, confMouth: 0, corridorBad: null, ptsMin: Infinity, ptsMax: 0 };
{
  ...
  const N = 20;
  const _h = Array.from({length: N}, (_, i) => `#seed=g2-${i}&regions=24`);
  const _r = await batch(_h);
  for (let i = 0; i < N; i++) {
    const g = _r[i].gj;
    {
...
const T0 = (await genEngine("#seed=names&regions=24&ep=0")).gj;
const T8 = (await genEngine("#seed=names&regions=24&ep=10")).gj;
{
  ...
  // chain's own monotone drop — the river-flows-uphill check in stress.mjs's validate()
}
...

{
  ...
  for (let i = 0; i < 10; i++) {
    const g = (await genEngine(`#seed=nm${i}&regions=24`)).gj;
...

{
  ...
  for (let i = 0; i < 6; i++) {
    const g = (await genEngine(`#seed=e6-${i}&regions=24`)).gj;
    for (const s of settlesOf(g)) {
      totalN++;
  ...
  const E = (await genEngine("#seed=vesper-9&regions=24&ep=8")).gj;
...

{
  ...
  // structural validate() in stress.mjs verifies the smuggling column ships.
  ...
  const A0 = (await genEngine("#seed=l1t-2&regions=24&ep=0")).gj;
  const A1b = (await genEngine("#seed=l1t-2&regions=24&ep=6&iq=90&hb=0&gt=70&db=10")).gj;
  const A2b = (await genEngine("#seed=l1t-2&regions=24&ep=0&capital=5")).gj;
  ...
  const S = (await genEngine("#seed=l1t-1&regions=24&ep=10&hb=0")).gj;
...

{
  ...
  const R = await genEngine("#seed=chain111&regions=24&ep=10");
...
const rClamp = await genEngine("#seed=z&regions=999&we=999&wg=-5");
...
{
  const Agt = await genEngine(BASE + "&gt=90");
...
// (ii) Neutral zero: gt=0 => the conduit reaches everyone.
{
  let allOn = true;
  for (let i = 0; i < 6; i++) {
    const g = (await genEngine(`#seed=nz${i}&regions=24&gt=0`)).gj;
...
// (iii) Default threshold: darkness exists, and off-grid tracks pop x wealth.
{
  ...
  const _h2942 = Array.from({length: N}, (_, i) => `#seed=dark${i}&regions=24`);
  const _r2942 = await batch(_h2942);
  for (let i = 0; i < N; i++) {
    const g = _r2942[i].gj;
    const regions = regionsOf(g);
...
// (iv) Services follow the grid: on-grid regions out-service off-grid ones.
{
  ...
  const _h2965 = Array.from({length: N}, (_, i) => `#seed=svc${i}&regions=24`);
  const _r2965 = await batch(_h2965);
  for (let i = 0; i < N; i++) {
    const g = _r2965[i].gj;
    const regions = regionsOf(g);
...
{
  const Adb = await genEngine(BASE + "&db=0");
...
// region sits at/near a refinery — the poison lands by geography.
{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=phys${i}&regions=24&db=0`)).gj;
    const regions = regionsOf(g);
...
// can no longer be counted on to put the blight on the poor. That is the whole point.
{
  ...
  for (let i = 0; i < N; i++) {
    for (const db of [15, 50, 85]) { // disperse, concentrate, treat
      const g = (await genEngine(`#seed=dbsw${i}&regions=20&db=${db}&ep=10`)).gj;
...
// (viii) B4 (#126): the three regimes, the db mapping, and the marquee exhibits.
{
  ...
  // 67–100 treat).
  const dDisp = (await genEngine("#seed=dmap&regions=20&db=10&ep=10")).gj.hinterland;
  const dConc = (await genEngine("#seed=dmap&regions=20&db=50&ep=10")).gj.hinterland;
  const dTreat = (await genEngine("#seed=dmap&regions=20&db=90&ep=10")).gj.hinterland;
  ...
  const _h3051 = Array.from({length: 24}, (_, i) => `#seed=dbz-${i}&regions=20&db=50&ep=10`);
  const _r3051 = await batch(_h3051);
  for (let i = 0; i < 24; i++) {
    const g = _r3051[i].gj;
  ...
  const _h3066 = Array.from({length: 32}, (_, i) => `#seed=dbz-${i}&regions=20&db=50&ep=10`);
  const _r3066 = await batch(_h3066);
  for (let i = 0; i < 32; i++) {
    const { gj, series } = _r3066[i];
  ...
  const _h3084 = Array.from({length: 24}, (_, i) => `#seed=dbz-${i}&regions=20&db=15&ep=10`);
  const _r3084 = await batch(_h3084);
  for (let i = 0; i < 24; i++) {
    const R = regsOf(_r3084[i].gj).filter(r => r.is_settled);
...
// must re-measure rather than assume. Recorded in CHANGELOG and docs/grounding.md §6.
{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=eo-${i}&regions=12&ep=10`)).gj;
...
// condition/is_decayed; findings carry crossings_total/decayed + trade_drag.
{
  ...
  const g0 = (await genEngine("#seed=am-8&regions=24&ep=0")).gj;
...
// (ii) THE MECHANIC across a sweep: an amnesty starves upkeep and the spans rot.
{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=am-${i}&regions=24&ep=10&iq=100`)).gj;
    const F = g.hinterland.findings;
...
{
  const g = (await genEngine("#seed=am-8&regions=24&ep=10&iq=100")).gj;
  const F = g.hinterland.findings;
  const amn = (g.hinterland.events || []).some(e => e.type === "reform" && e.measure === "toll_amnesty");
...
// adjustment), where a listening one reforms itself. reform_edges carries the ledger.
{
  ...
  const g0 = (await genEngine("#seed=le-2&regions=24&ep=0")).gj;
...
// it governs itself (reforms, never an imposition). The class of governance FLIPS.
{
  ...
  for (let i = 0; i < N; i++) {
    const lo = (await genEngine(`#seed=le-${i}&regions=24&ep=10&iq=0`)).gj;
    const hi = (await genEngine(`#seed=le-${i}&regions=24&ep=10&iq=100`)).gj;
...
// a `won_arc` column, and the chronicle/epithet/turning consumers learn both.
{
  ...
  const g0 = (await genEngine("#seed=ris-0&regions=24&ep=0")).gj;
  ...
  const flourA = [], starvA = [];
  const _h3332 = Array.from({length: N}, (_, i) => `#seed=ris-${i}&regions=24&ep=10`);
  const _r3332 = await batch(_h3332);
  for (let i = 0; i < N; i++) {
    const g = _r3332[i].gj;
...
{
  const flourChron = (await genEngine("#seed=ris-0&regions=24&ep=10")).chron;
  const starvChron = (await genEngine("#seed=ris-6&regions=24&ep=10")).chron; // B10 #132 re-pin: the mix shift moved which seed starves
...
// occupation is a local +50). Neutral at 50, so the default world is untouched.
{
  ...
  const a = await genEngine("#seed=alpha&regions=24&ep=10");
  const b = await genEngine("#seed=alpha&regions=24&ep=10&order=50");
...
// Predation falls and mobility falls together as order rises — the shared root.
{
  ...
  for (let i = 0; i < N; i++) {
    const open = regionsOf((await genEngine(`#seed=ord-${i}&regions=24&ep=10&order=0`)).gj).map(f => f.properties).filter(r => r.is_settled);
    const pol = regionsOf((await genEngine(`#seed=ord-${i}&regions=24&ep=10&order=100`)).gj).map(f => f.properties).filter(r => r.is_settled);
...
// capital. `hb` retires into `openness`; `bias` folds into legacy. Old links forward.
{
  ...
  for (let i = 0; i < N; i++) {
    const R = regionsOf((await genEngine(`#seed=tp-${i}&regions=24&ep=10&wt=80&we=8&wf=6&wg=6`)).gj).map(f => f.properties).filter(r => r.is_settled);
...
// each weight makes its OWN anchor the strongest predictor of wealth (§7.2 reach).
{
  ...
  const measure = async (cfg, feat) => {
    let s = 0, k = 8;
    for (let i = 0; i < k; i++) {
      const R = regionsOf((await genEngine(`#seed=mr-${i}&regions=24&ep=10&${cfg}`)).gj).map(f => f.properties).filter(r => r.is_settled);
...
// fleet LIMIT case; a named RIVAL rides the regime chain, the gazette, diplomacy.
{
  ...
  const g0 = (await genEngine("#seed=alpha&regions=24&ep=0")).gj;
  ...
  const CLS = {};
  const _h = Array.from({length: N}, (_, i) => `#seed=ir-${i}&regions=24&ep=10`);
  const _r = await batch(_h);
  for (let i = 0; i < N; i++) {
    const R = _r[i];
    const F = R.gj.hinterland.findings;
...
console.log("# The re-skin C1 (#134): the arcane-industrial register + new name registers");
{
  ...
  for (let i = 0; i < 10; i++) {
    const inst = (await genEngine(`#seed=reg-${i}&regions=24&ep=10`)).gj.hinterland.institutions;
...
// (viii) Emergence directions: burden rises with blight, falls with reach & wealth.
{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=hb${i}&regions=24`)).gj;
...
// (ix) The high-burden/low-care quadrant is populated (the payload claim).
{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=hq${i}&regions=24`)).gj;
    const regions = regionsOf(g);
...
// (x) Coverage: off-grid regions have larger service gaps than on-grid ones.
{
  let good = 0, tested = 0; const N = 15;
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=cg${i}&regions=24`)).gj;
    const regions = regionsOf(g);
...

{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=gov${i}&regions=24`)).gj;
    const regions = regionsOf(g);
...

{
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (offGrid.length > 0) offRoadOK++; // connectivity already asserted in stress.mjs's validate()
  }
  maCorr /= N;
...

{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=sw${i}&regions=24`)).gj;
    const regions = regionsOf(g);
...

{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=dt${i}&regions=24`)).gj;
    const regions = regionsOf(g);
    const P = (r) => r.properties;
...

{
  ...
  const E0 = (await genEngine("#seed=time0&regions=24&ep=0")).gj;
  const E8a = (await genEngine("#seed=time0&regions=24&ep=8")).gj;
  const E8b = (await genEngine("#seed=time0&regions=24&ep=8")).gj;
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=time${i}&regions=24&ep=8`)).gj;
    const regions = regionsOf(g);
    const P = (r) => r.properties;
...

{
  ...
  const N = 20;
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=hist${i}&regions=24&ep=10`)).gj;
    const regions = regionsOf(g);
...

{
  ...
  const _h3934 = Array.from({length: N}, (_, i) => `#seed=cf${i}&regions=24&ep=10`);
  const _r3934 = await batch(_h3934);
  for (let i = 0; i < N; i++) {
    const g = _r3934[i].gj;
...
// pre-chain engine produced NO war on this seed at all.
{
  ...
  const g = (await genEngine("#seed=chain63&regions=24&ep=10")).gj;
  const evs = g.hinterland.events || [];
  const strike = evs.find(ev => ev.type === "ore_strike");
  const war = evs.find(ev => ev.type === "war");
...

{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=inst${i}&regions=24&ep=10`)).gj;
    const regions = regionsOf(g);
    const byId = new Map(regions.map(r => [r.properties.region_id, r.properties]));
    const evs = g.hinterland.events || [];
...

{
  ...
  for (let i = 0; i < N; i++) {
    const g = (await genEngine(`#seed=st${i}&regions=24`)).gj;
    const regions = regionsOf(g);
    const P = (r) => r.properties;
...

{
  ...
  const Q1b = await genEngine("#seed=hinterland&regions=24&ep=8");
...
console.log("# The neutral instruments (#120, A3): the shape findings, two new lenses, the wealth boot");
{
  ...
  for (let i = 0; i < 31; i++) {
    const g = await genEngine(`#seed=a3-${i}&regions=18&ep=10`);
...
console.log("# The fate seed (#119, A2): same rock, different luck");
{
  ...
  const base = await genEngine("#seed=fateseed&regions=16&ep=10");
  const same = await genEngine("#seed=fateseed&fate=fateseed&regions=16&ep=10");
  ...
  const g0a = await genEngine("#seed=fateseed&fate=alpha&regions=16");
  const g0b = await genEngine("#seed=fateseed&fate=bravo&regions=16");
  ...
  const fA = await genEngine("#seed=fateseed&fate=alpha&regions=16&ep=10");
  const fB = await genEngine("#seed=fateseed&fate=bravo&regions=16&ep=10");
  ...
  const r1 = await genEngine("#seed=fateseed&fate=alpha&regions=16&ep=10");
  const r2 = await genEngine("#seed=fateseed&fate=alpha&regions=16&ep=10");
...
console.log("# The artifice index (#123, B1): the pie can grow — total wealth is no longer conserved");
{
  ...
  const Db1 = await genEngine("#seed=b1&regions=18&ep=10");
  ...
  for (let i = 0; i < 20; i++) {
    const d = (await genEngine(`#seed=piv-${i}&regions=18&ep=10`)).gj;
    if (totW(d) - totW0(d) < -3) defShrank++;
    const b = (await genEngine(`#seed=piv-${i}&world=era-26&regions=18&ep=10`)).gj;
  ...
  for (let i = 0; i < 8; i++) {
    const b = (await genEngine(`#seed=g-${i}&world=era-26&regions=18&ep=10`)).gj;
    if (totW(b) > totW0(b)) boomGrew++;
    const u = (await genEngine(`#seed=g-${i}&world=era-49&regions=18&ep=10`)).gj;
  ...
  for (let i = 0; i < 30 && !exhibitSeed; i++) {
    const P = regsOf((await genEngine(`#seed=piv-${i}&regions=18&ep=10`)).gj);
...
console.log("# The investment pool (#124, B2): the counting house's two edges — development finance vs comprador extraction");
{
  ...
  const Db2 = regsOf((await genEngine("#seed=b2&regions=20&ep=10")).gj);
  ...
  for (let i = 0; i < 16; i++) {
    const g = (await genEngine(`#seed=e-${i}&world=era-26&regions=22&ep=10`)).gj;
  ...
  for (let i = 0; i < 12; i++) {
    for (const c of regsOf((await genEngine(`#seed=e-${i}&world=era-49&regions=22&ep=10`)).gj)
  ...
  const quads = new Set();
  for (let i = 0; i < 24; i++) {
    const f = (await genEngine(`#seed=vd-${i}&regions=22&ep=10`)).gj.hinterland.findings;
...
console.log("# Migration both ways (#125, B3): the frontier term, emigration off-map, remittances home");
{
  ...
  const Db3 = regsOf((await genEngine("#seed=b3&regions=22&ep=10")).gj);
  ...
  for (let i = 0; i < 24; i++) {
    const R = regsOf((await genEngine(`#seed=fr-${i}&regions=22&ep=10`)).gj);
  ...
  let dipFloor = 0;
  for (let i = 0; i < 24; i++) {
    if (regsOf((await genEngine(`#seed=em-${i}&regions=22&ep=10`)).gj).some(r => {
...
console.log("# The world's shape (#122, B0.5): the 1600×1000 rectangle, no W stragglers");
{
  ...
  const Dshape = await genEngine("#seed=shape&regions=18&ep=10");
...
console.log("# The world outside (#121, B0): a third seed — the region is ruined or rescued by a history it cannot touch");
{
  ...
  const D = await genEngine("#seed=wo&regions=18&ep=10");
  ...
  const r1 = await genEngine("#seed=wo&fate=zz&world=venture&regions=18&ep=10");
  const r2 = await genEngine("#seed=wo&fate=zz&world=venture&regions=18&ep=10");
  if (JSON.stringify(r1.gj) === JSON.stringify(r2.gj) && r1.chron === r2.chron)
  ...
  const A = await genEngine("#seed=wo&world=alpha&regions=18&ep=10");
  const B = await genEngine("#seed=wo&world=bravo&regions=18&ep=10");
  ...
  const z1 = await genEngine("#seed=wo&world=alpha&regions=18");
  const z2 = await genEngine("#seed=wo&world=bravo&regions=18");
  ...
  const runLens = [];
  const _h4728 = Array.from({length: 40}, (_, i) => `#seed=wo&world=w-${i}&regions=12&ep=10`);
  const _r4728 = await batch(_h4728);
  for (let i = 0; i < 40; i++) {
    const chain = _r4728[i].gj.hinterland.world.regime_chain;
    let run = 1;
  ...
  for (const wl of ["concordat", "alpha", "bravo", "charlie", "delta", "echo", "foxtrot", "golf", "hotel", "india"])
    outcomes.push(meanW((await genEngine(seed2.replace("keystone", "keystone&world=" + wl))).gj));
  ...
  for (let s = 0; s < 8; s++) {
    ...
    for (const wl of ["concordat", "alpha", "bravo", "charlie", "delta", "echo"])
      sigs.add(evSig((await genEngine(`#seed=evshift-${s}&world=${wl}&regions=20&ep=10`)).gj));
...
console.log("# The suite audits itself: no assertion may sit in dead code");
{
  ...
  // with 284 assertions in it and no call site anywhere. It had been inert long
  // enough for its fertility mirror to fall two engine revisions behind, and three
  // comments elsewhere still claimed it ran. Nothing failed, because nothing ran.
  //
  // A test suite's worst failure mode is not a wrong assertion, it is an assertion
  // that cannot fail. So: every top-level function declared in the suite files must
  // be referenced somewhere other than its own declaration.
  // Referenced ACROSS the corpus, not just within a file: an exported helper is
  // legitimately called from a sibling, and counting per-file would flag every one.
  const files = ["test.mjs", "stress.mjs", "lib.mjs", "fixtures.matrix.mjs",
    "refixture.mjs", "atlas.mjs", "sweep.mjs", "worker.mjs", "pool.mjs", "bundle.mjs", "targets.mjs"];
  const corpus = new Map();
  for (const f of files) {
    try { corpus.set(f, readFileSync(new URL("./" + f, import.meta.url), "utf8")); } catch { /* optional file */ }
  }
  ...
  for (const [f, src] of corpus) {
    for (const m of src.matchAll(/^(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)/gm)) {
      const name = m[1];
      const refs = [...allText.matchAll(new RegExp("\\b" + name + "\\b", "g"))].length;
      if (refs <= 1) orphans.push(`${f}: ${name}()`);
    }
  }
  if (orphans.length === 0)
    ok(`no assertion sits in dead code: every top-level function in ${files.length} suite files is reachable`);
  else
    fail(`dead code in the suite, its assertions cannot fail: ${orphans.join(", ")}`);
}
...
console.log("# The golden fixtures (#118, A1): every export byte-pinned; a moved world must be a declared act");
{
  ...
  for (const cell of matrix) {
    ...
    // one event-loop breath per world, same memory discipline as genEngine()
    await new Promise((r) => setImmediate(r));
    if (typeof global !== "undefined" && global.gc) global.gc();
  }
  if (matrix.length !== 30)
