...
const d3 = globalThis.d3;
    ...
    function render(model, params) {
      ...
      const zc = labelZoom();
      const cz = (base, dp) => { const v = zc === 1 ? base : +(base * zc).toFixed(dp == null ? 2 : dp); return Math.max(6, v); };
      ...
      const labelBox = (x, y, text, fsize) => {
        ...
        const efs = fsize * zc;
        const w = String(text).length * efs * 0.62, h = efs;
      ...
      model.settlements.forEach(s => {
        ...
        const gap = Math.max(5, r * 0.3);
        placedLabels.push([labelBox(s.x, fy(s.y) - r - gap, s.name, fs), 4]);
      });
      // pre-reserve works-town + frontier-post boxes at rank 3 so curved
      // region labels (also rank 3) don't suppress settlement names
      model.settlements.forEach(s => {
        if (s.tier !== "works-town" && s.tier !== "frontier-post") return;
        if (zc >= 1) return; // only at deeper zoom where these labels appear
        const sreg = model.regions[s.regionId];
        const fs = 10 + 8 * Math.sqrt(RP(sreg) / maxPop0);
        const r = 3 + 13 * Math.sqrt(RP(sreg) / maxPop0);
        const gap2 = Math.max(5, r * 0.3);
        placedLabels.push([labelBox(s.x, fy(s.y) - r - gap2, s.name, fs), 3]);
      });
      model.regions.forEach(reg => {
        ...
        if (!atlas && SHOW_NUMBERS) { // the one number the map exists to show: halo + contrast, never a ghost
          ...
          parts.push(`<text x="${reg.c[0].toFixed(1)}" y="${(fy(reg.c[1]) + cz(26)).toFixed(1)}" font-size="${cz(12)}" font-weight="400" opacity="0.85" text-anchor="middle" fill="${ink}" paint-order="stroke" stroke="${halo}" stroke-width="${cz(3)}">${cellValue(reg)}</text>`);
        }
      ...
      if (!atlas && poi) {
        ...
        model.regions.forEach(reg => {
          ...
          // Ensure left-to-right path so textPath renders upright:
          // swap rays if ra leads to the right of rb.
          if (Math.cos(bestPair.ra.angle) > Math.cos(bestPair.rb.angle)) {
            const tmp = bestPair.ra; bestPair.ra = bestPair.rb; bestPair.rb = tmp;
          }
        ...
        rayScores.forEach(rs => {
          ...
          if (tryLabel(box, 3)) {
            ...
            parts.push(`<text font-size="${cz(fs)}" fill="#2a221a" opacity="0.72"><textPath href="#${lblId}" startOffset="50%" text-anchor="middle">${esc(rs.name)}</textPath></text>`);
          }
        });
      }
        ...
        if (S.name && poi) {
          ...
          if (Math.abs(aa) > 1e-6) { cx /= 3 * aa; cy /= 3 * aa;
            ...
            if (tryLabel(labelBox(cx, fy(cy), S.name, 15), 2)) {
              ...
              parts.push(`<text font-size="${cz(15)}" font-style="italic" letter-spacing="${Math.min(6, 2 + spread / 60).toFixed(1)}" fill="#1d4f7a" opacity="0.78" paint-order="stroke" stroke="#fff" stroke-width="${cz(3)}"><textPath href="#${arcId}" startOffset="50%" text-anchor="middle">${esc(S.name)}</textPath></text>`);
            }
          }
        }
      });
      ...
      // Forest tint: warm olive-green overlay on forest-biome regions,
      // land-clipped so trees never render on water.
      const FOREST_FILL = atlas ? "#8a9a6e" : "#7a8a5e";
      const FOREST_OPACITY = atlas ? 0.35 : 0.25;
      const forestEls = [];
      model.regions.forEach(reg => {
        if (reg.biome !== "forest") return;
        const dF = "M" + reg.ring.map(p => `${p[0].toFixed(1)},${fy(p[1]).toFixed(1)}`).join("L") + "Z";
        forestEls.push(`<path d="${dF}" fill="${FOREST_FILL}" fill-opacity="${FOREST_OPACITY.toFixed(2)}" stroke="none"/>`);
      });
      if (forestEls.length) parts.push(`<g class="forest-fill">${forestEls.join("")}</g>`);
      // Atlas-mode tree scatter: OSM broadleaved-style symbols (rounded canopy
      // + thin trunk, 4x8 px) placed within forest polygons via rejection
      // sampling. Reads as organic texture, not individual icons.
      if (atlas) {
        const treeSvgs = [];
        const TREE_SPACING = 20;
        const TREE = "M2,0C0.9,0 0,1.1 0,2.4C0,3.6 0.8,4.6 1.8,4.7L1.8,8L2.2,8L2.2,4.7C3.2,4.6 4,3.6 4,2.4C4,1.1 3.1,0 2,0Z";
        model.regions.forEach(reg => {
          if (reg.biome !== "forest") return;
          let xMn=Infinity,xMx=-Infinity,yMn=Infinity,yMx=-Infinity;
          for (const p of reg.ring){xMn=Math.min(xMn,p[0]);xMx=Math.max(xMx,p[0]);yMn=Math.min(yMn,p[1]);yMx=Math.max(yMx,p[1]);}
          const w=xMx-xMn,h=yMx-yMn;
          const budget=Math.max(3,Math.min(40,Math.round(w*h/1800)));
          const placed=[];
          const s=streams(params.seed)("tree"+reg.id);
          for(let t=0;t<budget;t++){
            let bx=0,by=0,ok=false;
            for(let a=0;a<25;a++){
              const tx=xMn+s()*w,ty=yMn+s()*h;
              if(!pointInRing(tx,ty,reg.ring))continue;
              let tc=false;
              for(const pp of placed){if(Math.hypot(tx-pp[0],ty-pp[1])<TREE_SPACING){tc=true;break;}}
              if(tc)continue;
              bx=tx;by=ty;ok=true;break;
            }
            if(!ok)continue;
            placed.push([bx,by]);
            treeSvgs.push(`<path d="${TREE}" transform="translate(${bx.toFixed(1)},${fy(by).toFixed(1)})" fill="#6b8552" opacity="0.55"/>`);
          }
        });
        if(treeSvgs.length) parts.push(`<g class="forest-trees">${treeSvgs.join("")}</g>`);
      }
      ...
      model.rivers.forEach(RV => {
        ...
        if (poi) {
          ...
          if (tryLabel(labelBox(mc[0], fy(mc[1]) + 18, rlbl, 12), 1)) {
            parts.push(`<text x="${mc[0].toFixed(1)}" y="${(fy(mc[1]) + 18).toFixed(1)}" font-size="${cz(12)}" font-style="italic" text-anchor="middle" fill="${atlas ? "#1a3a5c" : "#3d7ab5"}" paint-order="stroke" stroke="#fff" stroke-width="${cz(3)}">${RV.kind === "River" ? "the " + esc(RV.name) : esc(RV.name) + " " + esc(RV.kind)}</text>`);
          }
        }
      });
      ...
      if (atlas) model.ridges.forEach(R => {
        ...
        if (poi) {
          ...
          if (tryLabel(labelBox(mid[0], fy(mid[1]) - 10, R.name + " " + R.kind, 13), 1)) {
            parts.push(`<text x="${mid[0].toFixed(1)}" y="${(fy(mid[1]) - 10).toFixed(1)}" font-size="${cz(13)}" text-anchor="middle" fill="#4a3421" paint-order="stroke" stroke="#fff" stroke-width="${cz(3)}">${esc(R.name)} ${esc(R.kind)}</text>`);
          }
        }
      });
      ...
      if (atlas) model.passes.forEach(p => {
        ...
        if (poi) parts.push(`<text x="${p.x.toFixed(1)}" y="${(fy(p.y) + 20).toFixed(1)}" font-size="${cz(10)}" text-anchor="middle" fill="#4a3421" paint-order="stroke" stroke="#fff" stroke-width="${cz(2.5)}" opacity="0.85">${esc(p.name)}</text>`);
      });
      ...
      if (poi && model.freeport) {
        ...
        parts.push(`<circle cx="${P[0].toFixed(1)}" cy="${(fy(P[1]) + 6).toFixed(1)}" r="8" fill="none" stroke="#1a1a1a" stroke-width="2" opacity="0.8"/>`);
        parts.push(`<text x="${P[0].toFixed(1)}" y="${(fy(P[1]) + 21).toFixed(1)}" font-size="${cz(10)}" text-anchor="middle" fill="#1a1a1a" paint-order="stroke" stroke="#fff" stroke-width="${cz(2.5)}">${esc(model.freeport.name)}</text>`);
      }
      if (poi && model.sanctuary) {
        ...
        parts.push(`<text x="${model.sanctuary.x.toFixed(1)}" y="${(fy(model.sanctuary.y) - 30).toFixed(1)}" font-size="${cz(10)}" text-anchor="middle" fill="#8a6d00" paint-order="stroke" stroke="#fff" stroke-width="${cz(2.5)}">${esc(model.sanctuary.name)}</text>`);
      }
      ...
      if (poi) {
        ...
        if (still.length && model.stillName) {
          ...
          parts.push(`<text x="${cx2.toFixed(1)}" y="${(fy(cy2) - 44).toFixed(1)}" font-size="${cz(12)}" letter-spacing="2" text-anchor="middle" fill="#556" opacity="0.85" paint-order="stroke" stroke="#fff" stroke-width="${cz(2.5)}">${esc(model.stillName)}</text>`);
        }
      }
      ...
      model.settlements.forEach(s => {
        ...
        const gap = Math.max(5, r * 0.3);
        const off = !RG(sreg);
        const fill = off ? "#8a8a8a" : (isPrime ? "#d62728" : (s.tier === "city" ? "#fff" : (s.tier === "frontier-post" ? "#e0d8cc" : "#ccc")));
        const dash = off ? ` stroke-dasharray="4 3"` : "";
        // class="settle" marks the settlement dot itself. The map also draws
        // decorative rings (gate ownership, the freeport anchor, decayed
        // crossings, the selection halo), so "every circle" is not "every town";
        // the stress pin counts this class rather than subtracting the others.
        parts.push(`<circle class="settle" cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}" fill="${fill}" stroke="#000" stroke-width="${isPrime ? 2 : 1.2}"${dash} filter="url(#softShadow)"/>`);
        if (isPrime || s.tier === "city") {
          ...
          fsize = isPrime ? Math.min(Math.max(fsize, 20), 28) : Math.min(fsize, 18);
          const dfs = zc === 1 ? fsize.toFixed(1) : (fsize * zc).toFixed(1);
          parts.push(`<text class="placename" x="${cx.toFixed(1)}" y="${(cy - r - gap).toFixed(1)}" font-size="${dfs}" text-anchor="middle" fill="#000" font-weight="600" paint-order="stroke" stroke="#fff" stroke-width="${cz(4)}">${esc(s.name)}</text>`);
      ...
      // Wind arrow (top-left; display coords flip y)
      {
        ...
        parts.push(`<text x="${cx0}" y="${cy0 + 58}" font-size="${cz(14)}" text-anchor="middle" fill="#000" opacity="0.6" paint-order="stroke" stroke="#fff" stroke-width="3">wind</text>`);
      }
      ...
      // so the compass and the wind arrow are never read as one thing.
      {
        ...
        parts.push(`<text x="${nx}" y="${ny - 12}" font-size="${cz(13)}" font-weight="700" text-anchor="middle" fill="#000" opacity="0.7" paint-order="stroke" stroke="#fff" stroke-width="3">N</text>`);
      }
      if (params.ep > 0) {
        const ageName = ageAt(getFindings(model).ages, Math.min(scrubEpoch, snaps.length - 1));
        parts.push(`<text x="${WX - 40}" y="46" font-size="${cz(16)}" text-anchor="end" fill="#000" opacity="0.65" paint-order="stroke" stroke="#fff" stroke-width="4">epoch ${Math.min(scrubEpoch, snaps.length - 1)}/${params.ep}${ageName ? " · " + ageName : ""}</text>`);
      }
      ...
      if (!atlas) {
        ...
        parts.push(`<g class="scalebar" transform="translate(${sbX.toFixed(0)},${sbY.toFixed(0)})">
          ...
          <text x="${(sbW/2).toFixed(0)}" y="-10" text-anchor="middle" font-size="${cz(10)}" fill="#5c5045" opacity="0.8">140</text>
        </g>`);
      }
      parts.push(`</svg>`);
    ...
    const FEATURE_META = {
      ...
      freeport: { get: () => model.regions.find(r => r.id === model.freeport.regionId) || model.regions[0],
        name: () => model.freeport.name, sub: () => "freeport · a harbor beyond any writ", cap: "THE FREE PORT",
        rows: (r, row) => row("writ", "no crown charter, no temple toll — its trade enters no ledger") + row("sea reach", `${r.seaAccess}`) },
      sanctuary: { get: () => model.regions.find(r => r.id === model.sanctuary.regionId) || model.regions[0],
        name: () => model.sanctuary.name, sub: () => "high sanctuary · refuge above the sanctioned faith", cap: "THE SANCTUARY",
        rows: (r, row) => row("heals", "without a charter — a healer the planner never rationed") + row("legibility", `hides ${r.legibility} from the census`) },
      peak: { get: (id) => model.regions[id],
        name: (r) => (model.settlements.find(s => s.regionId === r.id) || { name: r.placeName }).name + " summit", sub: (r) => `peak · spot height ${r.elevation}`, cap: "THE SUMMIT",
        rows: (r, row) => row("elevation", `${r.elevation}`) + row("ruggedness", `${r.ruggedness}`) + row("biome", `${r.biome}`) },
      skyport: { get: (id) => model.regions[id],
        name: (r) => ((model.settlements.find(s => s.regionId === r.id) || {}).name || r.placeName) + " Aerie", sub: () => "skyport · an aerie of the skyway", cap: "THE AERIE",
        rows: (r, row) => row("sky advantage", `${r.skyAdvantage}`) + row("capital by air", `${r.seatCostSky}`) },
      camp: { get: (id) => model.regions[id],
        name: (r) => (model.settlements.find(s => s.regionId === r.id) || { name: r.placeName }).name + " camp", sub: () => "hunter camp · where the beasts are worth a bounty", cap: "THE CAMP",
        rows: (r, row) => row("predation", `−18 from camp ground`) + row("mobility", `+4 — risk is a wage`) },
      refinery: { get: (id) => model.regions[id],
        name: (r) => ((model.settlements.find(s => s.regionId === r.id) || {}).name || r.placeName) + " aetherworks", sub: () => "refinery · turns aetherstone into lumen", cap: "THE AETHERWORKS",
        rows: (r, row) => row("capacity", `${r.refining}`) + row("blight", `spoil load ${r.blight}`) },
      still: { get: (id) => model.regions[id],
        name: () => model.stillName, sub: () => "stillair · ground where the lift-stones die", cap: "THE STILL",
        rows: (r, row) => row("skyway", "no aerie can be chartered — a wall no money crosses") },
      garrison: { get: (id) => model.regions[id],
        name: (r) => ((model.settlements.find(s => s.regionId === r.id) || {}).name || r.placeName) + " post", sub: () => "constabulary post · the crown's force on the ground", cap: "THE POST",
        rows: (r, row) => row("strength", `${r.wardline}`) + row("force projection", `${r.forceProjection}`) },
      event: { get: (id) => model.regions[id],
        name: (r) => r.eventType ? r.eventType.replace(/_/g, " ") : "event", sub: (r) => `event · ${1000 + 25 * (r.eventEpoch || 0)}`, cap: "THE EVENT",
        rows: (r, row) => row("type", `${r.eventType}`) + row("severity", `${r.eventSeverity}`) + row("epoch", `${r.eventEpoch}`) },
      conduit: { get: (i) => model.conduitEdges[i],
        name: (e) => `grid line ${model.regions[e.a].id}–${model.regions[e.b].id}`, sub: (e) => `conduit · ${e.cls || "branch"} line`, cap: "THE CONDUIT",
        rows: (e, row) => row("class", `${e.cls || "branch"}`) + row("from", (model.settlements.find(s => s.regionId === model.regions[e.a].id) || { name: model.regions[e.a].placeName }).name) + row("to", (model.settlements.find(s => s.regionId === model.regions[e.b].id) || { name: model.regions[e.b].placeName }).name) },
      road: { get: (i) => model.roadEdges[i],
        name: (e) => `${e.name || "road"} (${e.cls || "track"})`, sub: (e) => `road · traffic ${e.traffic || 0}`, cap: "THE ROAD",
        rows: (e, row) => row("class", `${e.cls || "track"}`) + row("traffic", `${e.traffic || 0}`) + row("from", (model.settlements.find(s => s.regionId === model.regions[e.a].id) || {}).name || model.regions[e.a].placeName) + row("to", (model.settlements.find(s => s.regionId === model.regions[e.b].id) || {}).name || model.regions[e.b].placeName) },
    };
    ...
    function renderInspector() {
      ...
      // THE FULL LEDGER: every exported column, with a short description of
      // what it measures and how. Collapsed by default — click to expand.
      L.push(`<div class="insp-sec" onclick="
        const t=this.nextElementSibling;t.style.display=t.style.display==='none'?'':'none';
        this.textContent=t.style.display==='none'?'THE FULL LEDGER ▸':'THE FULL LEDGER ▾'
      " style="cursor:pointer">THE FULL LEDGER ▸</div>`);
      L.push(`<table style="display:none;font-size:13px;line-height:1.3;max-height:420px;overflow-y:auto">` +
        row("region_id", `${reg.id} — stable id within the file`) +
        row("wealth", `${reg.wealth} — emergent blend of extraction, artifice, and trade income`) +
        row("is_capital_region", `${reg.isCapital ? 1 : 0} — region containing the metropolis`) +
        row("population", `${reg.population} — settlement + rural population`) +
        row("pop_density", `${reg.popDensity} — persons per 100×100 cell`) +
        row("aetherstone_endowment", `${reg.endowment} — ore richness: blind geology`) +
        row("artifice_index", `${reg.A} — command of magically-enabled productivity`) +
        row("artifice_index_t0", `${reg.A0} — the works as founded`) +
        row("terrain_ruggedness", `${reg.ruggedness} — blind geology; feeds travel friction`) +
        row("fertility", `${reg.fertility} — derived from climate, water, elevation, biome`) +
        row("elevation", `${reg.elevation} — blind geology; blight flows downhill`) +
        row("biome", `${reg.biome} — ordered rules from elevation, rain, temp`) +
        row("biome_habitability", `${reg.biomeHabitability} — per-biome land quality (grassland=80, alpine=10)`) +
        row("biome_move_cost", `${reg.biomeMoveCost} — per-biome movement friction`) +
        row("water_access", `${reg.waterAccess} — surface + aquifer; income precondition`) +
        row("centrality_to_capital", `${reg.centrality} — inverted cost-distance from the capital`) +
        row("aetherworks_capacity", `${reg.refining} — 0 except refinery regions`) +
        row("value_retention", `${reg.retention} — share of value that stays local`) +
        row("on_grid", `${reg.onConduit ? 1 : 0} — wired to the lumen grid`) +
        row("grid_access", `${reg.conduitAccess} — 100 when wired; decays off-wire`) +
        row("arcane_service_index", `${reg.arcaneServices} — metered magic; needs grid + wealth`) +
        row("blight_load", `${reg.blight} — aetherworks plumes + disposal doctrine spoil`) +
        row("injustice_idx", `${reg.injustice} — labeled composite (blight × poverty)`) +
        row("healing_reach", `${reg.healingReach} — decay to nearest healer`) +
        row("safe_water", `${reg.safeWater} — waterworks/grid/wealth minus blight`) +
        row("vulnerability_idx", `${reg.vulnerability} — poverty + peripherality + tier`) +
        row("disease_burden_per_1k", `${reg.burden} — total = env + water + unmet causes`) +
        row("service_gap_idx", `${reg.serviceGap} — inverse reach + facility distance + off-grid`) +
        row("dominant_bloc", `${reg.occupied ? "dominion" : reg.bloc} — argmax of crown/temple/magnate`) +
        row("market_access", `${reg.marketAccess} — Hansen index over road-network costs`) +
        row("pilgrim_flux", `${reg.pilgrimFlux} — pilgrim traffic to sanctioned sites`) +
        row("force_projection", `${reg.forceProjection} — how far the state can throw force`) +
        row("order_level", `${reg.orderLevel} — realm order + local police state`) +
        row("constabulary_strength", `${reg.wardline} — strategic priority × lumen`) +
        row("security_status", `${reg.security} — thresholded on force projection`) +
        row("smuggling_intensity", `${reg.smuggling} — value routed around patrolled ground`) +
        row("predation_risk", `${reg.predation} — traffic × absence of protection`) +
        row("black_market_index", `${reg.blackMarket} — reliance on unsanctioned channels`) +
        row("enforcement_gap", `${reg.enforceGap} — illicit pressure minus state capacity`) +
        row("exhausted_lode", `${reg.exhausted ? 1 : 0} — ore mined out, no income today`) +
        row("founding_era", `${reg.era} — when the land was first settled`) +
        row("founding_age", `${reg.foundingAge}`) +
        row("legacy_advantage", `${reg.legacy} — head starts from founding age × grid`) +
        row("shock_legacy", `${reg.shock} — calamity that scarred this ground`) +
        row("shock_severity", `${reg.shockSeverity}`) +
        row("abandonment_index", `${reg.abandonment} — hysteresis: past value minus now`) +
        row("tenure_churn", `${reg.tenureChurn} — how often the land changed hands`) +
        row("segregation_index", `${reg.segregation} — wealth standing above neighbors`) +
        row("mobility_ceiling", `${reg.mobility} — chain-role × services × market access`) +
        row("social_trust", `${reg.socialTrust} — claiming the state vs routing around it`) +
        row("kinship_reliance", `${reg.kinship} — designed mirror of social_trust`) +
        row("cultural_distance", `${reg.culturalDistance} — distance from dominant culture`) +
        row("tenure_regime", `${reg.tenure} — whose land the registry recognizes`) +
        row("legibility_gap", `${reg.legibility} — how badly the census undercounts`) +
        row("uncounted_population", `${reg.uncounted} — add to pop for corrected rates`) +
        row("endowment_t0", `${reg.endowment0} — founding ore stock`) +
        row("wealth_t0", `${reg.wealthT0} — founding wealth`) +
        row("population_t0", `${Math.round(reg.popT0 * (1 + reg.rural))}`) +
        row("peak_wealth", `${reg.peakWealth} — high-water mark`) +
        row("ore_depleted", `${reg.oreDepleted ? 1 : 0} — mine died during the run`) +
        row("boom_bust", `${reg.boomBust} — settlement trajectory`) +
        row("won_arc", `${reg.wonArc || "null"} — what a won rising bought`) +
        row("elite_share", `${reg.eliteShare} — owners' row share of coin`) +
        row("elite_pop_pct", `${reg.elitePopPct} — owners' row headcount %`) +
        row("class_gap", `${reg.classGap} — coin-per-owner ÷ coin-per-laborer`) +
        row("tariff_burden", `${reg.tollBurden} — tariff at held gates`) +
    ...
    function hitTestFeature(wx, wy) {
      ...
      if (poi) {
        ...
        model.regions.filter(r => r.elevation >= 70).forEach((r, i) => tryPt(r.c[0], r.c[1], "peak", r.id, PT));
        // slot-based glyphs — approximate via region centroid
        model.regions.forEach(reg => {
        ...
        model.conduitEdges.forEach((e, i) => {
          const A = model.regions[e.a], B = model.regions[e.b];
          const d = nearPoly([[A.c[0], A.c[1]], [B.c[0], B.c[1]]]);
          if (d < LN && d < bestD) { bestD = d; best = { kind: "conduit", id: i }; }
        });
        // road edges — heavier traffic wins ties
        model.roadEdges.forEach((e, i) => {
          const A = model.regions[e.a], B = model.regions[e.b];
          const d = nearPoly([[A.c[0], A.c[1]], [B.c[0], B.c[1]]]);
          if (d < LN && d < bestD) { bestD = d; best = { kind: "road", id: i }; }
        });
      }
