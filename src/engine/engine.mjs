...
const esc = (s) => String(s).replace(/[&<>"']/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
    ...
    // `params` is optional and second, so the one existing caller keeps working:
    // when it is absent the voices come from whatever toGeoJSON already primed for
    // this model, which is the normal path (the app builds the export before it
    // offers the tables). A caller that wants the tables and nothing else passes
    // params and pays for one export.
    ...
    // Whatever this model's voices are: the primed set if the export has been built
    // (the normal path), a fresh build if the caller supplied params, otherwise
    // nothing rather than a guess.
    function voicesAny(model, params) {
      const per = VOICES_CACHE.get(model);
      if (per && per.size) return per.values().next().value;
      return params ? voicesFor(model, params) : [];
    }
