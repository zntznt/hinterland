...
**The experiment:** CLOSE THE COAST with `openness=0` (the old `hb=0` maps forward).
...
It is an emergent hierarchy, yes, but a built-in one; its **steepness** is the
...
| the artifice index `A` (B1) | the growth channel: income no longer conserved, so a boom grows the pie, a trade war shrinks it; a high-`A` periphery can out-earn the capital | `artifice_index`, `findings.growth` |
