...
| elite share | ratchets up structurally; falls only on catastrophe | **an ordinary `r − g` drift** (#166): concentration where returns on holdings outrun per-capita growth, **compression** where growth outruns them, with competition and boom-entry folded into `g` and no threshold gating either sign; the catastrophes stay on a separate discrete ledger | the `r − g` gap: holdings intensity; market access; order level; credit openness |
