...
5. **The F3 check (defeat is an institution):** the winter after every war
   ...
   than the war does.
6. **The F2 check (money begets reach begets money):** the provenance now
...
7. **The F1 check (the blocs become agents):** the chokepoints are now
   ...
   the final state.
8. **The P1 check (the wild layer):** everything on the map before this was
...
9. **The G3 check (the double lottery):** the map now has TWO geographic
...
10. **The G2 check (who drinks first):** overlay the **river** lines on a
...
11. **The G1 check (geography is destiny):** overlay the **ridge** lines and
...
12. **The E3 check (the world names itself):** categorize settlements on
...
13. **The D6 check (events cause events):** history is no longer a set of
...
14. **The D3 check (history with dates):** the provenance member now carries an
...
15. **The D2 check (watch it happen):** set epochs to 8+, click **Download
...
16. **The D1 check (time makes the loops real):** export the same seed at
   ...
   fixed world.
17. **The W4 check (the uncounted):** choropleth `legibility_gap` — the census
...
18. **The W3 check (the past sits on the land):** choropleth
...
19. **The W2 check (the shadow is the state's negative image):** choropleth
...
20. **The W1 check (two networks, one lie):** style roads by `road_class`
...
21. **The Phase 6 check (who governs whom):** categorize regions on
   ...
   auditable.
22. **The Phase 5 check (the payload — who gets sick, who gets care):**
...
23. **The Phase 4 check (environmental injustice):** choropleth `blight_load`
   ...
   of the injustice.
24. **The Phase 3 check (off-grid darkness):** style regions by
...
25. **The Phase 2 check (the resource curse):** scatter or bivariate
